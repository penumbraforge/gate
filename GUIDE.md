...
2. **Pattern Matching** — Tests each line against the 80 built-in regex rules plus the deduplicated FORTRESS pack loaded from `rules/rules.json`
...
The `rules/` directory contains the signed rule pack:
- **`rules.json`** — 68 detection patterns, cryptographically signed. At load
  time these are deduplicated against the built-ins (by id and pattern), so the
  unique survivors are added to the live rule set — see the count above.
...
None of the output formats emit the raw secret. The `match` field is redacted
(e.g. `sk_l****`) and each finding carries a `secretHash` (SHA-256 of the secret)
(a 12-char SHA-256 prefix) so occurrences can be correlated without exposing the value.
...
# Credential verification: make read-only API calls to confirm detected
# secrets are still live. Opt-in and OFF by default; enabling it here has the
# same effect as passing --verify (skipped when running as a pre-commit hook).
verify: false
...
| `verify` | `false` | Opt-in credential verification (live API checks); same as `--verify` |
...
gate vault encrypt "my-secret" # encrypt a value → self-contained vault blob
gate vault decrypt "VAULT:..." # decrypt a vault blob
gate vault env .env            # encrypt all values in a .env file
```

The encryption key is stored at `~/.gate/vault.key`. There are two reference forms:

- **Self-contained blob** — `gate vault encrypt` and `gate vault env` emit
  `VAULT:<base64>`, where the base64 payload is an AES-256-GCM record
  (`{iv, ct, authTag}`). Everything needed to decrypt (except the key) travels
  with the reference.
- **Store reference** — the interactive **Vault** action persists the full
  ciphertext to `~/.gate/vault.json` and leaves a short `VAULT:<id>` reference
  in source. The secret stays recoverable by id from the store.
...
The log is rotated once it passes 5MB: `audit.jsonl` → `audit.jsonl.1` → `audit.jsonl.2` (two generations kept).

---

## GitHub Action

The action source lives in `github-action/action.js` and is bundled with
[`@vercel/ncc`](https://github.com/vercel/ncc) into `github-action/dist/index.js`
(committed, so `@actions/*` deps ship inside it). The action metadata is
`action.yml` at the repo root, which is what makes `uses: penumbraforge/gate@v2`
resolve. Run `npm run build:action` after changing `action.js`; CI fails if the
...
| `use-local` | `false` | Run the checked-out repo's own CLI (`node bin/gate.js`) instead of installing from npm — used to self-test unreleased versions |
...
```bash
npm test                       # all tests (530+ tests, 29 suites)
npm run test:cli               # CLI tests only
npx jest --watch               # watch mode
npx jest --coverage            # run with coverage thresholds enforced
```
...
| Rules loader | `src/cli/__tests__/rules-loader.test.js` | ~7 |
...
Enforced globally by `jest --coverage` (declared at the root of `jest.config.js`
— thresholds inside `projects[]` are silently ignored by Jest). Values sit a
couple points under measured coverage so routine changes are not blocked:
...
| **Updates** | On-demand version check against `gate.penumbraforge.com` (only on `gate update` / `gate status`, never during a commit) |
...
| `src/cli/updater.js` | Self-update checker (queries `gate.penumbraforge.com`, only on `gate update`/`gate status`) |
...
| `rules/rules.json` | 68 detection patterns (signed; deduplicated against built-ins at load) |
| `rules/fortress.js` | Rule engine CLI — sign, verify, test rules |
| `rules/rules.json.sig` | HMAC-SHA256 signature for rules.json |
| `github-action/action.js` | GitHub Action source (bundled with ncc into `github-action/dist/index.js`) |
| `action.yml` | Action metadata at repo root (so `uses: penumbraforge/gate@v2` resolves) |
...
| JSON | `.json` | `process.env.VAR_NAME` (default reference) |
| Dockerfile | `Dockerfile` | `$VAR_NAME` (ENV → ARG rewrite) |
...
The action set depends on exposure. For **LOCAL / COMMITTED / UNKNOWN** findings:

| Key | Action |
|-----|--------|
| `f` | Fix — extract to `.env`, rewrite source line |
| `v` | Vault — encrypt and replace the secret with a `VAULT:<id>` reference |
| `i` | Ignore — add a rule-scoped entry to `.gateignore` |
| `s` | Skip — leave this finding unchanged (it will appear next scan) |
| `p` | Previous — go back to the previous finding |
| `j` | Jump — jump to a finding by number |
| `?` | Explain — show the full remediation guide + compliance mapping |
| `Ctrl+C` | Abort — exit immediately |

For **PUSHED** findings (secret already on a remote), `v`/`i` are replaced by `r`:

| Key | Action |
|-----|--------|
| `r` | Respond — start the guided incident-response workflow (recommended) |
| `f` | Fix code — extract from source (NOT sufficient alone once pushed) |
| `s` / `p` / `j` / `?` | Skip / Previous / Jump / Explain (as above) |

After the walkthrough, any skipped findings can be batch-added to `.gateignore`.
