...
function updateEnvFile(envPath, varName, value) {
  ...
  if (fs.existsSync(envPath)) {
    ...
    if (existing) {
      let existingValue = existing.split('=').slice(1).join('=').trim();
      ...
      if ((existingValue.startsWith('"') && existingValue.endsWith('"')) ||
        ...
        existingValue = existingValue.slice(1, -1);
      }
  ...
  const escaped = needsQuoting ? `"${value.replace(/\\/g, '\\\\').replace(/"/g, '\\"').replace(/\$/g, '\\$')}"` : value;
  content += `${varName}=${escaped}\n`;
  fs.writeFileSync(envPath, content, { mode: 0o600 });
...
function fixFinding(finding, filePath, options = {}) {
  ...
  if (rewritten !== null) {
    ...
    if (!dryRun) {
      ...
      result.importAdded = importResult.added || false;
...
function fixAll(scanResults, options = {}) {
  ...
  for (const fileResult of scanResults.filesScanned) {
    ...
    for (let j = 0; j < sortedFindings.length; j++) {
      const finding = sortedFindings[j];
      ...
      if (result.fixed) {
        ...
        if (result.importAdded) {
          for (let k = j + 1; k < sortedFindings.length; k++) {
            if (sortedFindings[k].lineNumber) {
              sortedFindings[k].lineNumber += 1;
            }
          }
        }
      } else {
